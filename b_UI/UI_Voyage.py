...
class voyage_UI :
    ...
    def register_voyage_UI(self):
        ...
        register = True 
        while register == True :
            flight_number = input("Enter a flight number (e.g NA0000): ")
            if self.la.check_flight_number(flight_number) == False :
                print("\nNot a valid flight number\n")
                break
            if self.la.flight_number(flight_number) == False :
            ...
            if self.la.check_time(year, month, day, hour,minute) == True :
                ...
                if self.la.check_flight_number(flight_number_return) == False :
                    print("\nNot a valid flight_number\n")
                    break
                if self.la.flight_number(flight_number_return) == False :
                    print("\nFlight number already in use!\n")
                    break
    ...
    def create_crew_air_voyage(self):
        ...
        while create == True :
            ...
            if self.la.check_flight_number(flightNumber) == False :
                print("\nNot a valid flight_number\n")
                break
            if self.la.flight_number(flightNumber) == True :
                print("\nNo flight available with this number!\n")
                break
